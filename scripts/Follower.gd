...
@export var spacing = 30 #Spacing from the preceding follower
...
func _process(delta: float) -> void:
	...
	if last_move:
		...
		_set_sprite(last_move.dir, idle)
	else:
		_set_sprite(6, true)
...
func _follow_preceding_follower(delta: float) -> void:
	...
	if dist_to_preceding > spacing: #If preceding node is not too close
...
func _set_sprite(dir_index: int, idle: bool = false) -> void:
